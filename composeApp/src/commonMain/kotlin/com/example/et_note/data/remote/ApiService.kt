...
class ApiService(val client: HttpClient, val dataStoreManager: DataStoreManager) {
    ...
    suspend fun login(request: AuthRequest): Result<AuthResponse>{
        return try {
            ...
            if (response.status == HttpStatusCode.OK) Result.success(response.body() as AuthResponse)
    ...
    suspend fun signup(request: AuthRequest): Result<AuthResponse>{
        return try {
            ...
            if (response.status == HttpStatusCode.Created) Result.success(response.body() as AuthResponse)
            else Result.failure(Exception("Failed to load data"))
        }catch (ex: Exception){
            Result.failure(ex)
        }
